...
$netCoreHost = Get-wmiobject -class win32_product | Where-Object {$_.Name -match "Microsoft .NET Core Host - 5.0" }
if([string]::IsNullOrEmpty($netCoreHost)) {
    Write-Host "Please install the hosting bundle and then restart the installation"
    Start-Process "https://dotnet.microsoft.com/download/dotnet-core/thank-you/runtime-aspnetcore-5.0.12-windows-hosting-bundle-installer"
    exit 1
}
